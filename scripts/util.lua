...
function util.transfer_burner_direct(burner_a, burner_b)
  if burner_a and burner_b then
    ...
    if burner_a.inventory and burner_b.inventory then
      util.swap_inventory(burner_a.inventory, burner_b.inventory)
    end
    if burner_a.burnt_result_inventory and burner_b.burnt_result_inventory then
      util.swap_inventory(burner_a.burnt_result_inventory, burner_b.burnt_result_inventory)
    end
  end
end
...
function util.copy_equipment_grid(entity_a, entity_b)
  if not (entity_a.grid and entity_b.grid) then
    return
  end
...
function util.orientation_to_direction(orientation)
  ...
  if orientation <= 0.5 then
    if orientation <= 0.25 then
      return orientation <= 0.125 and defines.direction.north or defines.direction.northeast
    else -- 0.25 < orientation <= 0.5
      return orientation <= 0.375 and defines.direction.east or defines.direction.southeast
    end
  else -- 0.5 < orientation
    if orientation <= 0.75 then
      return orientation <= 0.625 and defines.direction.south or defines.direction.southwest
    else -- 0.75 < orientation <= 1
      return orientation <= 0.875 and defines.direction.west or defines.direction.northwest
    end
  end
end
...
function util.overwrite_table(table_weak, table_strong)
  for k, v in pairs(table_strong) do
    table_weak[k] = v
  end
  return table_weak
end
