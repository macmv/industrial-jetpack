...
function data_util.transition_icons(icon_from, icon_to)
  if icon_to.icon then -- single 'to' icon format
    return {
      ...
      { icon = icon_from.icon, scale = 0.66 * 32 / icon_from.icon_size, shift = {8, -8}, icon_size = icon_from.icon_size },
      { icon = icon_to.icon, scale = 0.66 * 32 / icon_to.icon_size, shift = {-8, 8}, icon_size = icon_to.icon_size },
    ...
    for _, icon_and_size in pairs(icon_to) do
      table.insert(icons, { icon = icon_and_size.icon, scale = 0.5 * 32 / icon_and_size.icon_size, shift = {-12 + (_-1) * 8, 12}, icon_size = icon_and_size.icon_size })
    end
...
function data_util.auto_sr_hr(hr_version)
  ...
  sr_version.filename = data_util.replace(sr_version.filename, "__space-exploration-hr-graphics__", "__space-exploration-graphics__")
  sr_version.hr_version = data_util.hr(hr_version)
  return sr_version
end
